...
def test_educational_programs_loading():
    ...
    try:
        ...
        if rag_service.educational_programs_data:
            print(
                f"✅ Educational programs data loaded: {len(rag_service.educational_programs_data)} characters"
            )
...
def test_conversation_service():
    ...
    try:
        ...
        conv_service.store_message(
            test_user_id, "user", "Какие программы по ИИ есть в ИТМО?"
        )
        conv_service.store_message(
            test_user_id,
            "bot",
            'В ИТМО есть две магистерские программы по ИИ: "Искусственный интеллект" и "Управление ИИ-продуктами".',
        )
        ...
        if conversations:
            print(
                f"✅ Conversation service working: {len(conversations)} exchanges retrieved"
            )
...
def test_three_phase_context():
    ...
    try:
        ...
        educational_data = (
            rag_service.educational_programs_data or "Test educational data"
        )
        conversation_context = [
            {
                "user": {
                    "content": "Какие дисциплины есть в программе ИИ?",
                    "timestamp": 1234567890,
                },
                "bot": {
                    "content": "В программе ИИ есть множество дисциплин...",
...
def main():
    ...
    if passed == total:
        print(
            "🎉 All tests passed! Educational consultant integration is working correctly."
        )
        return True
    else:
