...
class FullIntegrationTest:
    ...
    def test_conversation_service(self):
        ...
        try:
            ...
            # Store user message
            success = service.store_message(test_user_id, 'user', 'Test question about ITMO programs')
            if not success:
                raise Exception("Failed to store user message")
            
            # Store bot response
            success = service.store_message(test_user_id, 'bot', 'Test response about educational programs')
            if not success:
    ...
    def test_rag_service_initialization(self):
        ...
        try:
            ...
            with patch('services.rag_service.QdrantClient') as mock_qdrant, \
                 patch('services.rag_service.EmbeddingService') as mock_embedding, \
                 patch('services.rag_service.RerankerService') as mock_reranker, \
                ...
                if rag_service.conversation_service.db_path != self.test_db_path:
                    raise Exception(f"RAG service using wrong database path: {rag_service.conversation_service.db_path}")
    ...
    def test_bot_initialization(self):
        ...
        try:
            # Mock external dependencies
            with patch('bot.RAGService') as mock_rag, \
                 patch('bot.ConversationService') as mock_conv:
    ...
    def test_educational_consultant_context(self):
        ...
        try:
            ...
            try:
                ...
                service.store_message(test_user_id, 'user', 'Расскажи о программе ИИ в ИТМО')
                service.store_message(test_user_id, 'bot', 'Программа "Искусственный интеллект" в ИТМО - это 2-летняя магистратура...')
    ...
    def run_all_tests(self):
        ...
        try:
            ...
            tests = [
                ...
                ("Educational Consultant Context", self.test_educational_consultant_context),
            ]
