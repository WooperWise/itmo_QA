...
from utils.message_utils import prepare_messages_with_sources, filter_think_blocks, log_full_response, clean_think_sections
from utils.logging_utils import setup_logging, log_user_interaction, log_error_with_context
from constants import (
    TELEGRAM_BOT_TOKEN, MAX_MESSAGE_LENGTH, get_message
)
...
async def history_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        recent_exchanges = conversation_service.get_recent_conversations(user.id, limit=3)
        ...
        if len(history_text) > MAX_MESSAGE_LENGTH:
            parts = [history_text[i:i+MAX_MESSAGE_LENGTH] for i in range(0, len(history_text), MAX_MESSAGE_LENGTH)]
            for part in parts:
...
async def health_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user.id not in []:  # Add admin user IDs here if needed
        logger.warning(f"Non-admin user {user.username} ({user.id}) tried to access health")
        return
    ...
    for component, status in health_status.items():
        ...
        status_text += f"{emoji} {component.title()}: {'Healthy' if status else 'Unhealthy'}\n"
    
    # Add conversation service health
    conv_health = conversation_service.health_check()
    emoji = "✅" if conv_health else "❌"
    status_text += f"{emoji} Conversation Service: {'Healthy' if conv_health else 'Unhealthy'}\n"
...
async def handle_question(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    logger.info(f"User {user.username} ({user.id}) asked: '{query[:100]}...' in {language}")
    ...
    try:
        ...
        logger.info(f"Successfully answered question for {user.username} in {duration:.2f}s")
...
async def handle_service_error(update: Update, context: ContextTypes.DEFAULT_TYPE, 
                              error_type: str) -> None:
