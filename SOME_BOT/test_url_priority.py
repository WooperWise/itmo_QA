    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        try:
            ...
            prompt = reranker._create_reranking_prompt(
                query, test_case["content"], test_case["url"]
            )
            ...
            print(f"❌ ERROR: {e}")
            results.append(
                {"test_case": test_case, "error": str(e), "is_correct": False}
            )
        ...
        for r in results:
            if not r.get("is_correct", False):
                ...
                if "error" in r:
                    ...
                    print(
                        f"- {desc}: Score {r['final_score']:.2f} not in expected range {r['test_case']['expected_score_range']}"
                    )
...
def test_document_reranking():
    ...
    try:
        ...
        for i, doc in enumerate(reranked, 1):
            ...
            print(
                f"#{i}: rerank_score={rerank_score:.3f}, original_score={original_score:.3f}"
            )
            print(f"     URL: {url}")
        ...
        target_in_top_2 = sum(
            1 for url in target_urls if any(target in top_url for top_url in top_2_urls)
        )
