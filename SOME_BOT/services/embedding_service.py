...
import ollama
from constants import OLLAMA_EMBEDDING_MODEL, OLLAMA_HOST, OLLAMA_PORT
...
class EmbeddingService:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if embedding:
                    ...
                    logger.warning(
                        f"No embedding returned from Ollama for attempt {attempt + 1}"
                    )

            except Exception as e:
                duration = time.time() - start_time if "start_time" in locals() else 0
                logger.warning(
                    f"Embedding attempt {attempt + 1} failed after {duration:.2f}s: {e}"
                )
    ...
    def generate_embeddings_batch(
        ...
        logger.info(
            f"Successfully generated {successful_count}/{len(texts)} embeddings"
        )
