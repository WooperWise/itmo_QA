...
from constants import (
    ...
    VECTOR_SEARCH_LIMIT, RERANKER_LIMIT,
    TOP_EMBEDDER_RESULTS, TOP_RERANKER_RESULTS, MAX_CONTEXT_CHARS,
    TOP_CHUNKS_FOR_CONTEXT, TOP_COMPLETE_PAGES
)
from utils.logging_utils import (
    log_performance, log_error_with_context,
    log_vector_search_results, log_user_interaction
)
...
class RAGService:
    ...
    def __init__(self):
        ...
        self.target_programs_keywords = {
            'ru': [
                'искусственный интеллект', 'ии', 'машинное обучение', 'нейросети',
                'управление ии-продуктами', 'ии-продукты', 'продуктовый менеджмент',
                'artificial intelligence', 'ai product management', 'ai products'
            ],
            'en': [
                'artificial intelligence', 'ai', 'machine learning', 'neural networks',
                'ai product management', 'ai products', 'product management',
                'искусственный интеллект', 'управление ии-продуктами', 'ии-продукты'
            ]
        }
    ...
    def _load_educational_programs(self) -> None:
        ...
        try:
            ...
            if not educational_file_path.exists():
                logger.warning(f"Educational programs file not found: {educational_file_path}")
            ...
            logger.info(f"Loaded educational programs data: {len(self.educational_programs_data)} characters")
            
        except Exception as e:
            logger.error(f"Failed to load educational programs data: {e}")
            log_error_with_context(
                e,
                {'file_path': str(educational_file_path) if 'educational_file_path' in locals() else 'unknown'}
            )
            self.educational_programs_data = ""
    
    def _group_chunks_by_page(self, chunks: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
        """
        ...
        try:
            ...
            scroll_result = self.qdrant_client.scroll(
                ...
                scroll_filter=Filter(
                    should=[
                        FieldCondition(key="url", match=MatchValue(value=page_identifier)),
                        FieldCondition(key="page_info", match=MatchValue(value=page_identifier))
                    ]
                ),
    ...
    def reconstruct_complete_pages(self, page_identifiers: List[str]) -> Dict[str, Dict[str, Any]]:
        """
        ...
        for page_id in page_identifiers:
            try:
                ...
                logger.debug(f"Reconstructed page {page_id}: {len(combined_content)} chars from {len(page_chunks)} chunks")
    ...
    def _enhance_query_for_ai_programs(self, query: str, language: str = 'en') -> str:
        ...
        keywords = self.target_programs_keywords.get(language, self.target_programs_keywords['en'])
        ...
        if not has_ai_keywords:
            ...
            if language == 'ru':
                enhanced_query += " искусственный интеллект ИИ-продукты ИТМО магистратура"
            else:
                enhanced_query += " artificial intelligence AI products ITMO master's program"
        
        logger.debug(f"Enhanced query from '{query}' to '{enhanced_query}'")
        return enhanced_query
    
    def _prioritize_ai_program_results(self, results: List[Dict[str, Any]],
                                     query: str, language: str = 'en') -> List[Dict[str, Any]]:
        """
        ...
        logger.info(f"Prioritized {len(ai_program_results)} AI program results out of {len(results)} total")
    ...
    def vector_search(self, query_embedding: List[float], 
        ...
        try:
            ...
            logger.info(f"Vector search completed: {len(results)} results in {duration:.2f}s")
    ...
    @log_performance
    def answer_question(self, query: str, language: str = 'en',
                       user_id: int = None) -> Tuple[Optional[str], List[Dict[str, Any]]]:
        """
        ...
        try:
            logger.info(f"Processing question: '{query[:100]}...' in {language} for user {user_id}")
            ...
            if user_id:
                ...
                try:
                    conversation_context = self.conversation_service.get_recent_conversations(user_id, limit=3)
                    logger.info(f"Retrieved {len(conversation_context)} recent conversation exchanges")
                except Exception as e:
            ...
            search_results = self._prioritize_ai_program_results(search_results, query, language)
            
            # Log vector search results
            log_vector_search_results(search_results, enhanced_query)
            
            # Step 6: Extract top chunks for Block 3
            logger.debug(f"Step 6: Extracting top {TOP_CHUNKS_FOR_CONTEXT} chunks for context block 3")
            ...
            logger.info(f"Selected {len(reranked_pages)} complete pages after reranking")
            ...
            answer = self.llm_service.generate_answer_two_phase(
                query, top_chunks, reranked_pages, language,
            ...
            logger.info(f"Question answered successfully in {duration:.2f}s using three-phase approach")
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._ensure_qdrant_connection():
                collections = self.qdrant_client.get_collections()
                health_status['qdrant'] = QDRANT_COLLECTION_NAME in [c.name for c in collections.collections]
