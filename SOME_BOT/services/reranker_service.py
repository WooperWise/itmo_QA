...
class RerankerService:
    ...
    def _create_reranking_prompt(
        self, query: str, document: str, document_url: str = ""
    ) -> str:
        """
...
ОТВЕТ:"""
    ...
    def _get_url_based_score_boost(self, url: str) -> float:
        ...
        medium_priority_patterns = ["ai.itmo.ru", "artificial", "intelligence"]
    ...
    def rerank_documents(
        ...
        if page_groups:
            logger.info(
                f"Reranking {len(page_groups)} pages for query: '{query[:50]}...'"
            )
        ...
        log_reranker_results(
            ...
            query,
            duration,
            page_groups if "page_groups" in locals() else None,
        )

        logger.info(
            f"Reranking completed: selected {len(reranked_docs)} documents in {duration:.2f}s"
        )
    ...
    def _get_relevance_score(self, prompt: str, max_retries: int = 3) -> float:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if score is not None:
                    ...
                    logger.warning(
                        f"Attempt {attempt + 1}/{max_retries}: Could not extract score from response: '{response_text}'"
                    )

            except Exception as e:
                logger.warning(
                    f"Reranking attempt {attempt + 1}/{max_retries} failed: {e}"
                )
        ...
        logger.warning(
            f"All {max_retries} reranking attempts failed, returning neutral score"
        )
        return 5.0
        ...
        for i, pattern in enumerate(patterns):
            ...
            if match:
                try:
                    ...
                    logger.debug(
                        f"Extracted score {clamped_score} using pattern {i+1}: '{pattern}'"
                    )
    ...
    def health_check(self) -> bool:
        ...
        try:
            ...
            test_prompt = self._create_reranking_prompt(
                "test query about AI programs", "test document content", "test.url"
            )
    ...
    def _rerank_chunks(
        ...
        for i, doc in enumerate(documents):
            try:
                ...
                logger.debug(
                    f"Document {i+1}: base_score={base_score:.2f}, url_boost={url_boost:.2f}, final_score={final_score:.2f}, url='{url}'"
                )
    ...
    def _rerank_pages(
        self, query: str, page_groups: Dict[str, List[Dict[str, Any]]]
    ) -> List[Tuple[Dict[str, Any], float]]:
        """
        ...
        for page_id, chunks in page_groups.items():
            try:
                ...
                # Create specialized reranking prompt with URL
                prompt = self._create_reranking_prompt(
                    query, combined_content_truncated, url
                )
                ...
                logger.debug(
                    f"Page {page_id}: base_score={base_score:.2f}, url_boost={url_boost:.2f}, final_score={final_score:.2f}, chunks={len(chunks)}, url='{url}'"
                )
