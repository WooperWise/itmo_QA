...
class ConversationService:
    ...
    def store_message(self, user_id: int, message_type: str, content: str) -> bool:
        ...
        try:
            ...
            logger.debug(f"Stored {message_type} message for user {user_id}: {len(cleaned_content)} chars")
            return True
    ...
    @log_performance
    def get_recent_conversations(self, user_id: int, limit: int = 3) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            cursor = self.connection.execute("""
                ...
                LIMIT ?
            """, (user_id, limit * 2))  # Get more messages to ensure we have complete exchanges
            ...
            logger.info(f"Retrieved {len(recent_exchanges)} recent exchanges for user {user_id}")
    ...
    def clean_response(self, text: str) -> str:
        ...
        text = re.sub(r'<planning>.*?</planning>', '', text, flags=re.DOTALL | re.IGNORECASE)
        ...
        try:
            ...
            if stats and stats['total_messages'] > 0:
                return {
                    ...
                    'conversation_span_hours': (stats['last_message_time'] - stats['first_message_time']) / 3600 if stats['first_message_time'] else 0
                }
            else:
                return {
                    'total_messages': 0,
    ...
    def health_check(self) -> bool:
        ...
        try:
            ...
            cursor = self.connection.execute("SELECT COUNT(*) FROM conversations LIMIT 1")
            cursor.fetchone()
    ...
    def get_service_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cursor = self.connection.execute("SELECT COUNT(*) as total FROM conversations")
            total_conversations = cursor.fetchone()['total']
            
            # Get unique users count
            cursor = self.connection.execute("SELECT COUNT(DISTINCT user_id) as unique_users FROM conversations")
