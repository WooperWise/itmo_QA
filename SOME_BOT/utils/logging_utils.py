...
def log_vector_search_results(results: List[Dict[str, Any]], query: str) -> None:
    ...
    if not results:
        logger.warning(f"Vector search returned no results for query: '{query[:100]}...'")
        return
...
def log_reranker_results(original_results: List[Dict[str, Any]],
                        reranked_results: List[Dict[str, Any]],
                        query: str,
                        duration: float,
                        page_groups: Dict[str, List[Dict[str, Any]]] = None) -> None:
    """
    ...
    if page_groups:
        logger.info(f"Reranker: selected {len(reranked_results)} from {len(page_groups)} "
        ...
        for i, result in enumerate(reranked_results):
            ...
            logger.info(f"Reranked #{i+1}: rerank_score={score:.3f} (orig={original_score:.3f}), "
                        f"original_pos={original_position}, {chunks_in_page} chunks, {page_info}, {url}")
    else:
        logger.info(f"Reranker: selected {len(reranked_results)} from {len(original_results)} "
...
def log_generation_metrics(response: str, 
                          context_length: int, 
                          duration: float, 
                          language: str) -> None:
    """
    ...
    logger.info(f"Generated {response_length} chars ({words_count} words) "
...
def log_user_interaction(user_id: int, 
                        username: str, 
                        query: str, 
                        response_parts: int, 
    ...
    logger.info(f"User interaction: {username} (ID: {user_id}) asked '{query[:100]}...', "
...
def log_error_with_context(error: Exception, 
    ...
    logger.error(f"Error: {type(error).__name__}: {error}{user_info}, Context: {context_str}")