...
def format_sources(sources: List[dict], language: str = 'en') -> str:
    ...
    result = header + "\n" + "\n".join(source_lines)
    logger.debug(f"Formatted {len(sources)} sources (no deduplication) for language: {language}")
    
    return result


def prepare_messages_with_sources(text: str, sources: List[dict], language: str = 'en', max_length: int = 4000) -> List[str]:
    """
...
def log_full_response(filtered_answer: str, sources: List[dict], think_blocks: List[str]) -> None:
    """
