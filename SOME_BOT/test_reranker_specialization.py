...
class TestRerankerSpecialization:
    ...
    def create_test_document(
        self, content: str, url: str, page_info: str = ""
    ) -> Dict[str, Any]:
    ...
    def test_url_based_scoring(self):
        ...
        for url, expected_boost in test_cases:
            ...
            status = (
                "✅ PASS" if abs(actual_boost - expected_boost) < 0.01 else "❌ FAIL"
            )
            logger.info(
                f"{status} URL: '{url}' -> Expected: {expected_boost}, Got: {actual_boost}"
            )
        ...
        for response_text, expected_score in test_cases:
            ...
            if expected_score is None:
                ...
                status = (
                    ...
                    and abs(actual_score - expected_score) < 0.01
                    else "❌ FAIL"
                )

            logger.info(
                f"{status} Response: '{response_text}' -> Expected: {expected_score}, Got: {actual_score}"
            )
            ...
            self.test_results.append(
                {
                    ...
                    "actual": actual_score,
                    "passed": (expected_score is None and actual_score is None)
                    or (
    ...
    def test_document_ranking_simulation(self):
        ...
        ai_program_urls = [
            "abit.itmo.ru/program/master/ai",
            "abit.itmo.ru/program/master/ai_product",
        ]

        ai_programs_in_top_2 = sum(
            1 for url in ai_program_urls if any(ai_url in url for ai_url in top_2_urls)
        )
        ...
        if ai_programs_in_top_2 >= 2:
            ...
            logger.info(
                f"❌ FAIL Only {ai_programs_in_top_2} AI program URLs in top 2 positions"
            )
    ...
    def generate_test_summary(self):
        ...
        for test_type, stats in test_types.items():
            ...
            logger.info(
                f"  {test_type}: {stats['passed']}/{stats['total']} ({success_rate:.1f}%)"
            )
