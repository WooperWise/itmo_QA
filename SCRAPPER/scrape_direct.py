...
def print_result_summary(
    ...
    title: str = "Crawl Results Summary",
    max_items: int = 10,
):
    ...
    for result in results:
        ...
        if result.markdown:
            if hasattr(result.markdown, "raw_markdown"):
                ...
                content_info = (
                    f" | Markdown: [cyan]Present ({len(str(result.markdown))} chars)[/]"
                )
        elif result.html:
...
def save_results_to_markdown(
    results: List[CrawlResult], filename: str = "crawl_results.md"
):
...
async def deep_crawl_with_error_resistance(
    url: str, max_depth: int = 4, max_pages: int = 50
) -> List[CrawlResult]:
    """Perform a deep crawl with error resistance - continues even if individual pages fail."""
    console.print(
        f"[bold cyan]Starting deep crawl of {url} with max depth {max_depth} and max pages {max_pages}[/]"
    )
    ...
    config = CrawlerRunConfig(
        ...
        markdown_generator=DefaultMarkdownGenerator(
            content_filter=PruningContentFilter(
                threshold=0.6, threshold_type="relative"
            )
        ),
        verbose=True,
    )
            ...
            if isinstance(crawl_results, list):
                for i, result in enumerate(crawl_results):
                    try:
                        ...
                        if result.url in visited_urls:
                            console.print(
                                f"[yellow]![/] Skipping duplicate URL: {result.url}"
                            )
                            continue
                        ...
                        if result.success:
                            ...
                            console.print(
                                f"[green]✓[/] Successfully crawled: {result.url}"
                            )
                        else:
                        ...
                        console.print(
                            f"[yellow]![/] Skipping problematic result #{i} due to error: {str(e)}"
                        )
                        failed_pages.append(result)
            else:
                # Single result
                try:
                    ...
                    if crawl_results.url in visited_urls:
                        console.print(
                            f"[yellow]![/] Skipping duplicate URL: {crawl_results.url}"
                        )
                    else:
                        # Add URL to visited set
                        ...
                        if crawl_results.success:
                            ...
                            console.print(
                                f"[green]✓[/] Successfully crawled: {crawl_results.url}"
                            )
                        else:
                            failed_pages.append(crawl_results)
                            console.print(
                                f"[red]✗[/] Failed to crawl: {crawl_results.url}"
                            )
                except Exception as e:
                    console.print(
                        f"[yellow]![/] Skipping problematic result due to error: {str(e)}"
                    )
                    failed_pages.append(crawl_results)
    ...
    console.print(
        f"[blue]Duplicate pages skipped: {len(visited_urls) - len(results) - len(failed_pages)}[/]"
    )
...
async def main():
    ...
    if len(sys.argv) > 1:
        ...
        if len(sys.argv) > 2:
            try:
                ...
                console.print(
                    "[yellow]Invalid depth parameter. Using default value of 4.[/]"
                )
                max_depth = 4
        ...
        if len(sys.argv) > 3:
            try:
                ...
                console.print(
                    "[yellow]Invalid max_pages parameter. Using default value of 50.[/]"
                )
                max_pages = 50
    else:
        ...
        console.print(
            "[bold cyan]Enter the maximum number of pages (default is 50):[/]"
        )
    ...
    results = await deep_crawl_with_error_resistance(
