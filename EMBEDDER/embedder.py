...
def create_collection(client):
    ...
    try:
        ...
        logger.info(f"Created collection '{QDRANT_COLLECTION_NAME}' with vector size {VECTOR_SIZE}")

def get_ollama_client():
    """Initialize and return Ollama client."""
    return ollama.Client(host=f"http://{os.environ.get('OLLAMA_HOST', 'localhost')}:{os.environ.get('OLLAMA_PORT', 11434)}")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                logger.error(f"Failed to upload point after {max_retries} attempts: {e}")
                return False
...
def process_markdown_files():
    ...
    for idx, file_path in enumerate(all_files):
        ...
        if not page_info and not page_url:
            ...
            logger.info(f"No metadata found in {file_path.name}, using fallback: {page_info}")
            ...
            logger.info(f"No page info found in {file_path.name}, using fallback: {page_info}")
        
        # Split content into chunks with overlap
        chunks = chunk_content(full_content, CHUNK_SIZE, CHUNK_OVERLAP)
        logger.info(f"Split {file_path.name} into {len(chunks)} chunks with {CHUNK_OVERLAP} char overlap")
        ...
        for i, chunk in enumerate(chunks):
            try:
                ...
                logger.info(f"Generated embedding for {file_path.name} chunk {i+1}/{len(chunks)} (Global: {global_progress:.2f}%) (size: {len(embedding)})")
                ...
                if upload_point_with_retry(qdrant_client, point):
                    vector_count += 1
                    logger.info(f"Uploaded point for {file_path.name} chunk {i+1}/{len(chunks)} to Qdrant (Page: {page_info}, URL: {page_url})")
                else:
                    logger.error(f"Failed to upload point for {file_path.name} chunk {i+1}/{len(chunks)} to Qdrant")
                
            except Exception as e:
                processed_chunks += 1
                # Calculate and log global progress percentage even for failed chunks
                global_progress = (processed_chunks / total_chunks) * 100
                logger.error(f"Error generating embedding for {file_path.name} chunk {i+1}: {e} (Global: {global_progress:.2f}%)")
                continue
    
    logger.info(f"Processed {file_count} files and generated {vector_count} vectors with {CHUNK_OVERLAP} character overlap")
